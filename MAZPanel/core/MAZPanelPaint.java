...
public class MAZPanelPaint {
	...
	public void init(IPaintInstruction paintInstruction, int screenWidth, int screenHeight) {
		this.paintInstruction=paintInstruction; this.screenWidth=screenWidth; this.screenHeight=screenHeight;
	}
	
	public void paint(Graphics2D g) {
		g.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
		this.paintCoordinateLines(g);
		this.paintInstruction.paint(g); // Paint Dependency
		this.paintZoomPercent(g);
	}

	private void paintCoordinateLines(Graphics2D g) {
		if(this.coordinateOnTime > 0) { // Mask
			...
			Point2D.Double start = AffineTool.transformPoint(new Point(0, 0), g.getTransform());
			Point2D.Double  end =  AffineTool.transformPoint(new Point(this.screenWidth, this.screenHeight), g.getTransform());
			int aCoordWidth = (int) (end.getX() - start.getX()) + this.lineInterval*2; 
			int aCoordHeight = (int) (end.getY() - start.getY()) + this.lineInterval *2;
			...
			for(int i=-1; i< aCoordWidth +1; i+=this.lineInterval) {g.fill(new Rectangle((int)start.getX() - this.lineInterval + i - MAZConstant.LineWidth/2 + transX, (int)start.getY() + transY - this.lineInterval, MAZConstant.LineWidth, aCoordHeight));}
			for(int i=-1; i< aCoordHeight +1; i+=this.lineInterval) {g.fill(new Rectangle((int)start.getX() + transX - this.lineInterval, (int)start.getY() - this.lineInterval + i - MAZConstant.LineWidth/2 + transY, aCoordWidth, MAZConstant.LineWidth));}
		}
	}

	private void paintZoomPercent(Graphics2D g) {
		if(this.zoomPercentOnTime > 0) {
			...
			PaintTool.paintText(
					...
					new Rectangle2D.Double(this.screenWidth/2 - this.zoomBlockWidth/2, this.screenHeight/2 - this.zoomBlockheight/2, this.zoomBlockWidth, this.zoomBlockheight)
			);
		}
	}
